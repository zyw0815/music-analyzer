...
ffmpeg 用于音频转码，以及 DSF/DFF 等 DSD 文件的播放和分析支持。请安装完整的 ffmpeg 套件，不要只安装精简版、浏览器插件版或只包含 `ffprobe` 的包。
...
```bash
# macOS with Homebrew，安装完整 ffmpeg
brew install ffmpeg
```

```bash
# Windows with winget，安装 Gyan 完整构建
winget install Gyan.FFmpeg
```

```bash
# Ubuntu/Debian，安装发行版完整 ffmpeg 包
sudo apt-get update
sudo apt-get install -y ffmpeg
```

```bash
# Conda，也可以单独从 conda-forge 安装完整 ffmpeg
conda install -c conda-forge ffmpeg
```

如果使用项目提供的 Conda 环境，`backend/environment.yml` 已经包含 ffmpeg。
